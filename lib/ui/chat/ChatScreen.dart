...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget _myMessageContentWidget(MessageData messageData) {
    ...
    if (mediaUrl.contains('audio')) {
      ...
      return Stack(
          overflow: Overflow.visible,
          alignment: Directionality.of(context) == TextDirection.ltr ? Alignment
            ...
            Positioned.directional(
              ...
              child: Image.asset(
                Directionality.of(context) == TextDirection
                    .ltr ? 'assets/images/chat_arrow_right.png' :
                'assets/images/chat_arrow_left.png',
                color: Color(COLOR_ACCENT),
                height: 12,
              ),
  ...
  _sendMessage(String content, Url url, String videoThumbnail) async {
    ...
    if (url != null) {
      if (url.mime.contains('image')) {
        ...
        message.content = 'sentAVideo'.tr
          (args: ['${MyAppState.currentUser.firstName}']);
      } else if (url.mime.contains('audio')) {
        message.content = 'sentAVoiceMessage'.tr(
            args: ['${MyAppState.currentUser.firstName}']);
      }
    }
    if (await _checkChannelNullability(
      ...
      await _fireStoreUtils.sendMessage(
          ...
          homeConversationModel.isGroupChat,
          message,
          homeConversationModel.conversationModel, true);
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(context, 'block'.tr(),
                  'hasBeenBlocked.'.tr(args: ['${homeConversationModel
                      .members.first
                      .fullName()}'
                  ]));
            } else {
              _showAlertDialog(
                  ...
                  'block'.tr(),
                  'couldNotBlock'.tr(
                      args: ['${homeConversationModel.members.first
                          .fullName()}'
                      ]));
            }
          },
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(context, 'report'.tr(),
                  'hasBeenBlockedAndReported'.tr(args: ['${homeConversationModel
                      .members.first
                      .fullName()}'
                  ]));
            } else {
              _showAlertDialog(
                  context,
                  'report'.tr(),
                  'couldNotReportAndBlock'.tr(args: ['${homeConversationModel
                      .members.first
                      .fullName()}'
                  ]));
            }
